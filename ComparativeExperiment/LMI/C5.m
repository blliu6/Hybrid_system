...
pvar x1 x2;
vars = [x1, x2];
f = [x2; 0.333333333333333*x1^3 - x1 - x2];

prog = sosprogram(vars);
[prog, B] = sospolyvar(prog, monomials([x1; x2],[0,1,2]), 'wscoeff1');

init = [-(x1 - (1.0))*(x1-(2.0));-(x2 - (-0.5))*(x2-(0.5))];
unsafe = [-(x1 - (-1.399999976158142))*(x1-(-0.6000000238418579));-(x2 - (-1.399999976158142))*(x2-(-0.6000000238418579))];
inv = [-(x1 - (-3.5))*(x1-(2.0));-(x2 - (-2.0))*(x2-(1.0))];
